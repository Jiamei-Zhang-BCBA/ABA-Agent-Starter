...
## 📅 日期使用规范（OBS-02 防漂移）
`{{当前日期}}` = **今天执行的日期**。档案内任何引用具体事件的日期必须遵守：

1. **frontmatter 的 `date` / 新文件的文件名日期** → 可以用 `{{当前日期}}`
2. **档案正文里引用的事件日期**（如 "2026-04-23 那节课"、"上周三 ABC 事件"、
   数据窗口 / 评估执行日 / 访谈日）→ **必须**从用户上传材料或上下文档案里读出来，
   不允许用 `{{当前日期}}` 替代
3. **时间悖论自检**：产出前检查，写信 / 报告文件名上的日期**绝不能早于**文中
   引用的事件日期。如果材料里事件日期晚于今天（例如来自未来的课堂日志），
   保留原日期并在 frontmatter 加 `数据窗口截止: YYYY-MM-DD` 说明
4. 若材料无法明确某事件日期，写 `[日期待补]`，**不要**用今天兜底
...
## 🛑 真名零出现规则（BUG #24 — P1 安全铁律）
如果上传材料里含有真实人物姓名（父母全名、儿童真名、医生、班主任姓名等），
**你产出的所有 FILE marker 文件里都不得出现**这些真名。具体禁令：

- ❌ **正文**里不得写真名 — 一律用"父亲/母亲/外婆/班主任"角色代称
- ❌ **frontmatter** 里不得写真名
- ❌ **meta-commentary / 合规声明 / 脱敏说明** 里也**不得引用真名**
  解释"我脱敏了"是合规意图，**但只要引用了原始真名就是自毁式脱敏**。
- ❌ **姓氏 + 职业敬称变体**（李老师 / 陈教授 / 王医生等）如果指的是已知家庭成员，
  也必须替换为角色代称

**典型错误（v4 实际踩坑）**：
```
❌ 5. 原始资料中出现的父母姓名（李建国/陈婉仪）：已通过 privacy-filter 处理
```
上面这段想说"已脱敏"但实际把真名打进了正式档案。

**正确写法**：
```
✅ 5. 本报告使用角色代称（父亲/母亲/外婆）引用家庭成员。
```

自检：**产出前扫一遍**每个 FILE marker 下的文本，若命中上传材料里出现过的真名
或姓氏+敬称组合 → **立即删除或改写整句**。若不可避免提及"脱敏"事实，只说"本
报告使用角色代称"，**绝不引用原始真名**。
